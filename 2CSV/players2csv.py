...
output_directory = 'C:/Users/Noah Lecegui/Documents/Fantasy Insights/Archivos/CSV/Players'
...
def calculate_averages(player_stats_list):
    if not player_stats_list:
        return {field: 0 for field in ["mins_played", "goals", "goal_assist", "offtarget_att_assist",
                                        ...
                                        "won_contest", "ball_recovery", "poss_lost_all", "penalty_conceded"]}
    ...
    with tqdm(total=len(json_files), desc='Progreso', bar_format="{l_bar}{bar}|", dynamic_ncols=True) as pbar:
        for json_file in json_files:
            ...
            if not data.get("playerStats"):
                data["playerStats"] = [{
                    "stats": {
                        field: [0, 0] for field in ["mins_played", "goals", "goal_assist", "offtarget_att_assist",
                                                   "pen_area_entries", "penalty_won", "penalty_save", "saves",
                                                   "effective_clearance", "penalty_failed", "own_goals", "goals_conceded",
                                                   "yellow_card", "second_yellow_card", "red_card", "total_scoring_att",
                                                   "won_contest", "ball_recovery", "poss_lost_all", "penalty_conceded"]
            ...
            if player_stats_list:
                last_player_stats = player_stats_list[-1]
                week_number = last_player_stats.get("weekNumber", 0)
                week_points = last_player_stats.get("totalPoints", 0)
            ...
            df = pd.DataFrame({
                ...
                **averages,
                ...
                "weekNumber": [week_number],  # Añadir weekNumber aquí
                "weekPoints": [week_points],  # Añadir weekPoints aquí
